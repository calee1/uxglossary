...
function itemToCSVLine(item: GlossaryItem): string {
  ...
  const escapedAcronym = acronym.includes(",") || acronym.includes('"') ? `"${acronym.replace(/"/g, '""')}"` : acronym

  return `${item.letter},${item.term},${escapedDefinition},${escapedAcronym}`
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const itemToAdd = {
      ...
      acronym: newItem.acronym?.trim() || undefined,
    }
  ...
  try {
    ...
    try {
      ...
      console.log("Request body parsed:", {
        ...
        acronym: requestData.item?.acronym,
      })
    } catch (parseError) {
      console.error("Failed to parse request body:", parseError)
      return NextResponse.json({ error: "Invalid request body" }, { status: 400 })
    ...
    items[index] = {
      ...
      acronym: item.acronym?.trim() || undefined,
    }
