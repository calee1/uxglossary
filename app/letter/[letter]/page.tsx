...
async function loadGlossaryDataSafe() {
  try {
    ...
    console.log("Letter page - Available letters:", Object.keys(data))
    console.log("Letter page - Total terms:", Object.values(data).flat().length)
    return data
  } catch (error) {
...
export default async function LetterPage({ params }: LetterPageProps) {
  ...
  // Include numbers in our alphabet
  const alphabetWithNumbers = ["0", ...Array.from({ length: 26 }, (_, i) => String.fromCharCode(65 + i))]

  if (!alphabetWithNumbers.includes(letter) && letter !== "0-9") {
    notFound()
  }
  ...
  console.log(`Letter page - Letter "${letter}" has ${items.length} items`)
  ...
  return (
    <main className="max-w-4xl mx-auto p-4 sm:p-6 border border-gray-200 dark:border-gray-700 rounded-lg my-4 sm:my-8 bg-white dark:bg-gray-800 transition-colors">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link
          href={prevLetter ? `/letter/${prevLetter === "0" ? "0-9" : prevLetter.toLowerCase()}` : "#"}
        ...
        >
          ...
          <span className="text-sm sm:text-base">{prevLetter === "0" ? "0-9" : prevLetter || "Prev"}</span>
        </Link>
        ...
        <Link
          href={nextLetter ? `/letter/${nextLetter === "0" ? "0-9" : nextLetter.toLowerCase()}` : "#"}
          className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1 sm:py-2 rounded-lg transition-colors ${
            nextLetter
              ? "text-blue-500 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-blue-200 dark:border-blue-700"
              : "text-gray-400 dark:text-gray-500 cursor-not-allowed"
        ...
        >
          <span className="text-sm sm:text-base">{nextLetter === "0" ? "0-9" : nextLetter || "Next"}</span>
      ...
      <div className="flex justify-between items-center mt-8 sm:mt-12 pt-4 sm:pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        >
          ...
          <span className="text-xs sm:text-sm">
            {prevLetter ? `Letter ${prevLetter === "0" ? "0-9" : prevLetter}` : "Previous"}
          </span>
        </Link>
        ...
        >
          <span className="text-xs sm:text-sm">
            {nextLetter ? `Letter ${nextLetter === "0" ? "0-9" : nextLetter}` : "Next"}
          </span>
          <ChevronRight size={16} className="sm:w-5 sm:h-5" />
        </Link>
      </div>
